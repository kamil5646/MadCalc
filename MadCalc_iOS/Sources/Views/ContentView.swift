...
struct ContentView: View {
    ...
    private var hero: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Natywna aplikacja iOS do optymalizacji cięcia sztang. Działa offline, zapisuje dane lokalnie i pozwala wyeksportować raport PDF.")
                .font(.callout)
    ...
    private var settingsSection: some View {
        sectionCard(title: "Parametry", subtitle: "Domyślnie pracujesz w centymetrach, ale możesz przełączyć się na milimetry.") {
            VStack(spacing: 14) {
                ...
                HStack(spacing: 12) {
                    decimalField("Długość sztangi [\(viewModel.unit.label)]", text: $viewModel.stockLengthInput)
    ...
    private var resultSection: some View {
        sectionCard(title: "Wynik", subtitle: "Nazwij sztangi i wyeksportuj czytelny plan cięcia do PDF.") {
            if let result = viewModel.result, let generatedSettings = viewModel.generatedSettings {
                VStack(alignment: .leading, spacing: 14) {
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                        ...
                        metricCard("Wykorzystanie", value: "\(viewModel.formatPercent(result.utilizationPercent))%")
                    }

                    Text("Policzone dla sztangi \(viewModel.formatLength(generatedSettings.stockLengthMm)) i grubości piły \(viewModel.formatLength(generatedSettings.sawThicknessMm)).")
                        .font(.footnote)
                ...
                emptyState("Brak wyniku", "Dodaj elementy i kliknij „Generuj plan”, aby policzyć liczbę sztang oraz rozpiskę cięć.")
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Nr \(bar.barIndex) • \(bar.cutCount) elementów • piła \(viewModel.formatLength(viewModel.totalSawThickness(for: bar)))")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
