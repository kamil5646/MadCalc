...
final class MadCalcViewModel: ObservableObject {
    ...
    func exportPDF() {
        ...
        do {
            let url = try pdfBuilder.makeTemporaryReportURL(
                items: items,
            ...
            presentError("Nie udało się przygotować PDF.")
        }
    }
    ...
    private func convertDisplayedValue(_ value: String, from sourceUnit: MeasurementUnit, to targetUnit: MeasurementUnit) -> String {
    ...
    private func persist() {
        let settings = readSettings(showErrors: false) ?? CutSettings(stockLengthMm: 6000, sawThicknessMm: 3)
        persistence.save(
            PersistedState(
                unit: unit,
                items: items,
