...
struct PDFReportBuilder {
  ...
  func makeTemporaryReportURL(
    ...
    let reportsDirectory = FileManager.default.temporaryDirectory.appendingPathComponent(
      "MadCalcReports", isDirectory: true)
  ...
  private func makePDF(
    ...
    return renderer.pdfData { context in
      ...
      render.drawSubtitle("Raport optymalizacji cięcia")
      ...
      render.drawTable(
        ...
        rows: itemRows,
        columnFractions: [0.36, 0.20, 0.44],
        accent: PDFTheme.brandBlue,
...
private final class PDFRenderContext {
  ...
  func drawPanel(title: String, lines: [String], accent: UIColor = PDFTheme.brandBlueDark) {
    ...
    repeat {
      ...
      if y + minimumPanelHeight + PDFLayout.panelSpacing > PDFLayout.contentBottomY,
        y > PDFLayout.topInset
      {
        drawFooter()
        beginPage()
      ...
      let accentRect = CGRect(
        x: panelRect.minX, y: panelRect.minY, width: 6, height: panelRect.height)
      accent.setFill()
      UIBezierPath(
  ...
  func drawTable(
    ...
    while !remainingRows.isEmpty {
      ...
      let maxRowHeightOnFreshPage = max(
        minimumRowHeight,
        PDFLayout.contentBottomY
          - (PDFLayout.topInset + titleHeight + titleSpacing + headerHeight + bottomSpacing)
      )
      ...
      if y + titleHeight + titleSpacing + headerHeight + firstRowHeight + bottomSpacing
        > PDFLayout.contentBottomY,
        y > PDFLayout.topInset
      {
        drawFooter()
      ...
      var renderedRowOnPage = false
      while !remainingRows.isEmpty {
        let row = remainingRows[0]
        let availableHeight = PDFLayout.contentBottomY - y
        ...
        if rowHeight > availableHeight {
          if rowHeight > maxRowHeightOnFreshPage {
            let split = splitRow(
              row,
              font: cellFont,
              widths: columnWidths,
              paddingX: cellPaddingX,
              paddingY: cellPaddingY,
              maximumHeight: maxRowHeightOnFreshPage
            )
            let visibleRowHeight = measureRowHeight(
              cells: split.visible,
              font: cellFont,
              widths: columnWidths,
              paddingX: cellPaddingX,
            ...
            if visibleRowHeight <= availableHeight {
              ...
              if let remainder = split.remainder {
                remainingRows[0] = remainder
              } else {
                remainingRows.removeFirst()
              }

              tableRowIndex += 1
              renderedRowOnPage = true
              continue
            }
          }
  ...
  private func splitRow(
    ...
    let availableTextHeight = max(1, maximumHeight - paddingY * 2)
    var visibleCells: [String] = []
    var remainderCells: [String] = []
    var hasRemainder = false

    for (index, cell) in cells.enumerated() {
      let textWidth = max(1, widths[index] - paddingX * 2)
      let wrapped = wrappedLines(for: cell, font: font, width: textWidth)

      if wrapped.isEmpty {
        visibleCells.append("")
        remainderCells.append("")
        continue
      }

      var takenLines: [String] = []
      for line in wrapped {
        let candidateLines = takenLines + [line]
        let candidateText = candidateLines.joined(separator: "\n")
        let candidateHeight = measure(text: candidateText, font: font, width: textWidth)

        if candidateHeight <= availableTextHeight || takenLines.isEmpty {
          takenLines = candidateLines
        } else {
          break
        }
      }

      if takenLines.isEmpty {
        let truncated = truncateToFitSinglePage(
          wrapped[0],
          font: font,
          width: textWidth,
          maximumHeight: availableTextHeight
        )
        takenLines = [truncated.visible]
        if !truncated.remainder.isEmpty {
          remainderCells.append(truncated.remainder)
          hasRemainder = true
        } else {
          remainderCells.append("")
        }
        visibleCells.append(truncated.visible)
        continue
      }

      let visibleText = takenLines.joined(separator: "\n")
      visibleCells.append(visibleText)

      let consumedCount = takenLines.count
      let leftoverLines = Array(wrapped.dropFirst(consumedCount))
      if leftoverLines.isEmpty {
        remainderCells.append("")
      } else {
        remainderCells.append(leftoverLines.joined(separator: "\n"))
        hasRemainder = true
      }
    }

    return (visibleCells, hasRemainder ? remainderCells : nil)
  }
  ...
  private func wrappedLines(for text: String, font: UIFont, width: CGFloat) -> [String] {
    ...
    for paragraph in paragraphs {
      ...
      for word in paragraph.split(separator: " ", omittingEmptySubsequences: true).map(String.init)
      {
        ...
        if measureSingleLineWidth(candidate, font: font) <= width {
          currentLine = candidate
          continue
        }
        ...
        if measureSingleLineWidth(word, font: font) <= width {
          currentLine = word
        } else {
          let chunks = splitLongToken(word, font: font, width: width)
          if chunks.count > 1 {
            lines.append(contentsOf: chunks.dropLast())
          }
          currentLine = chunks.last ?? word
        }
      }
  ...
  private func splitLongToken(_ token: String, font: UIFont, width: CGFloat) -> [String] {
    var chunks: [String] = []
    var currentChunk = ""

    for character in token {
      let candidate = currentChunk + String(character)
      if measureSingleLineWidth(candidate, font: font) <= width || currentChunk.isEmpty {
    ...
    return chunks.isEmpty ? [token] : chunks
  }

  private func truncateToFitSinglePage(
    _ text: String,
    font: UIFont,
    width: CGFloat,
    maximumHeight: CGFloat
    ...
    while index < text.endIndex {
      ...
      let candidate = visible + String(text[index..<nextIndex])
      let candidateHeight = measure(text: candidate, font: font, width: width)
      if candidateHeight > maximumHeight, !visible.isEmpty {
        break
      }
      visible = candidate
      index = nextIndex
