...
struct CutOptimizer: Sendable {
  func optimize(items: [CutItem], settings: CutSettings) throws -> OptimizationResult {
    ...
    let utilizationPercent =
      totalStockMm == 0 ? 0 : (Double(totalUsedMm) / Double(totalStockMm)) * 100
  ...
  private func buildGreedyBars(cuts: [Int], settings: CutSettings) throws -> [BarPlan] {
    ...
    while !remaining.isEmpty {
      ...
      guard !selection.indices.isEmpty else {
        throw CutOptimizationError(
          message: "Nie udało się ułożyć planu cięcia dla podanych danych.")
      }
  ...
  private func findOptimalPacking(
    ...
    let adjustedCapacity = settings.stockLengthMm + settings.sawThicknessMm
    let groupedCuts = Dictionary(grouping: cuts, by: { $0 })
      .sorted(by: { $0.key > $1.key })
    let lengths = groupedCuts.map(\.key)
    let initialCounts = groupedCuts.map { $0.value.count }
    let adjustedWeights = lengths.map { $0 + settings.sawThicknessMm }
    ...
    var currentPatterns: [BarPattern] = []
    var patternCache: [PatternState: [BarPattern]] = [:]
    var bestBarsUsedForState: [PatternState: Int] = [:]

    func lowerBoundFor(_ counts: [Int]) -> Int {
      let remainingAdjustedWeight = zip(counts, adjustedWeights).reduce(0) { partial, pair in
        partial + (pair.0 * pair.1)
      }
      guard remainingAdjustedWeight > 0 else {
        return 0
      }
      return (remainingAdjustedWeight + adjustedCapacity - 1) / adjustedCapacity
    }

    func expand(pattern: BarPattern) -> [Int] {
      var bar: [Int] = []
      for (index, count) in pattern.counts.enumerated() where count > 0 {
        bar.append(contentsOf: Array(repeating: lengths[index], count: count))
      }
      return bar
    }

    func hasRemainingCutThatFits(
      remainingCounts: [Int],
      selectedCounts: [Int],
      remainingCapacity: Int
    ) -> Bool {
      for index in remainingCounts.indices {
    ...
    func isBetter(pattern candidate: BarPattern, than current: BarPattern) -> Bool {
      if candidate.adjustedUsed != current.adjustedUsed {
        return candidate.adjustedUsed > current.adjustedUsed
      }

      let candidatePieces = candidate.counts.reduce(0, +)
      let currentPieces = current.counts.reduce(0, +)
      if candidatePieces != currentPieces {
        return candidatePieces > currentPieces
      }

      for (left, right) in zip(candidate.counts, current.counts) where left != right {
        return left > right
      }
      return false
    }

    func patterns(for state: PatternState) -> [BarPattern] {
      if let cached = patternCache[state] {
        return cached
      }

      guard let anchorIndex = state.remainingCounts.firstIndex(where: { $0 > 0 }) else {
        return []
      }

      var selectedCounts = Array(repeating: 0, count: lengths.count)
      selectedCounts[anchorIndex] = 1
      var generated: [BarPattern] = []

      func buildPattern(_ index: Int, adjustedUsed: Int) {
        if index == lengths.count {
          let remainingCapacity = adjustedCapacity - adjustedUsed
          if !hasRemainingCutThatFits(
            remainingCounts: state.remainingCounts,
            selectedCounts: selectedCounts,
            remainingCapacity: remainingCapacity
          ) {
            generated.append(BarPattern(counts: selectedCounts, adjustedUsed: adjustedUsed))
          }
          return
        }

        let available = state.remainingCounts[index] - selectedCounts[index]
        let maxTake = min(available, (adjustedCapacity - adjustedUsed) / adjustedWeights[index])

        for take in stride(from: maxTake, through: 0, by: -1) {
          selectedCounts[index] += take
          buildPattern(index + 1, adjustedUsed: adjustedUsed + (take * adjustedWeights[index]))
          selectedCounts[index] -= take
        }
      }

      buildPattern(anchorIndex, adjustedUsed: adjustedWeights[anchorIndex])
      generated.sort { left, right in
        isBetter(pattern: left, than: right)
      }
      patternCache[state] = generated
      return generated
    }

    func search(_ state: PatternState, barsUsed: Int) {
      if barsUsed >= bestBarCount {
        return
      }

      guard state.remainingCounts.contains(where: { $0 > 0 }) else {
        bestBarCount = barsUsed
        bestPacking = currentPatterns.map(expand(pattern:))
        return
      }

      let stateLowerBound = lowerBoundFor(state.remainingCounts)
      guard barsUsed + stateLowerBound < bestBarCount else {
        return
      }

      if let bestSeen = bestBarsUsedForState[state], bestSeen <= barsUsed {
        return
      }
      bestBarsUsedForState[state] = barsUsed

      for pattern in patterns(for: state) {
        var nextCounts = state.remainingCounts
        for index in nextCounts.indices {
          nextCounts[index] -= pattern.counts[index]
        }

        currentPatterns.append(pattern)
        search(PatternState(remainingCounts: nextCounts), barsUsed: barsUsed + 1)
    ...
    search(PatternState(remainingCounts: initialCounts), barsUsed: 0)
...
private struct PatternState: Hashable {
  let remainingCounts: [Int]
}

private struct BarPattern {
  let counts: [Int]
  let adjustedUsed: Int
}
